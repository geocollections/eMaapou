...
function isValidFilter(filter: FilterUnion): boolean {
  switch (filter.type) {
    ...
    case "idList":
      return filter.value.length > 0;
    case "rangeAlt":
      return filter.value.some((value) => value !== undefined);
    case "geom": {
...
export const useLocalities = defineStore("localities", () => {
  ...
  const routeQuerySchemaFilters = z.object({
    ...
    stratigraphyAge: z
      ...
      .transform((val, ctx) => {
        ...
        return [start, end] as [undefined | number, undefined | number];
      })
  ...
  function getQueryParams() {
    ...
    if (filters.value.stratigraphyAge.value.some((v) => v !== undefined)) {
